  ...
  return (
    <div style={{ color: '#E8EAF0' }}>
      ...
      <div className="px-5 pt-12 pb-4" style={{ background: '#1E2330', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <p className="text-sm mt-1" style={{ color: '#8A92A8' }}>Ostatnie {completedSessions.length} treningów</p>
      </div>
      ...
      <div className="mx-5 mb-4 p-4 rounded-2xl" style={{ background: '#161920', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="text-xs font-semibold mb-3" style={{ color: '#8A92A8' }}>km tygodniowo</div>
          ...
          {['tyg-6', 'tyg-5', 'tyg-4', 'tyg-3', 'tyg-2', 'tyg-1'].map((label, i) => {
            ...
            return (
              <div key={label} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full rounded-t-lg transition-all" style={{ height: `${h}%`, background: i === 5 ? '#FF5C1B' : 'rgba(255,255,255,0.15)' }} />
                <div className="text-xs" style={{ color: '#8A92A8' }}>{`-${6 - i}W`}</div>
              </div>
            )
          })}
        </div>
      ...
      <div className="px-5 space-y-3">
        {completedSessions.map(session => {
          ...
          return (
            <div key={session.id} className="rounded-2xl overflow-hidden" style={{ background: '#161920', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              {fb && (
                <div className="px-4 py-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  <div className="text-xs mb-1" style={{ color: '#8A92A8' }}>
                    {fb.source === 'voice' ? '🎤' : fb.source === 'text' ? '✏️' : '⌚'} Twój feedback
                  </div>
                  <div className="text-xs line-clamp-2" style={{ color: '#E8EAF0' }}>{fb.aiSummary}</div>
                  {fb.coachReply && (
