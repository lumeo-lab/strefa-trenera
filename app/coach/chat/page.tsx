...
export default function CoachChatPage() {
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-64 border-r flex flex-col shrink-0" style={{ borderColor: 'rgba(255,255,255,0.07)', background: '#161920' }}>
        <div className="px-4 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
          <div className="font-semibold text-sm">Czat</div>
          <div className="text-xs mt-0.5" style={{ color: '#8A92A8' }}>Rozmowy z zawodnikami</div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs truncate" style={{ color: '#8A92A8' }}>{athlete.goal}</div>
                </div>
              </button>
            )
          })}
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.07)', background: 'rgba(13,15,20,0.9)' }}>
          {selectedAthlete && (
            <>
              <Avatar initials={selectedAthlete.avatar} size="sm" />
              <div>
                <div className="font-semibold text-sm">{selectedAthlete.name}</div>
                <div className="text-xs" style={{ color: '#8A92A8' }}>{selectedAthlete.package} · {selectedAthlete.goal}</div>
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {threadMessages.length === 0 && (
            <div className="text-center py-12 text-sm" style={{ color: '#8A92A8' }}>Brak wiadomości. Napisz pierwszą!</div>
          )}
              ...
              <div key={msg.id} className={`flex gap-3 ${isCoach ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-sm ${isCoach ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                  <div
                    ...
                    style={{
                      background: isCoach ? '#FF5C1B' : '#1E2330',
                      color: isCoach ? 'white' : '#E8EAF0',
